...
The `monitor` and `lb` fields are used to define the health monitor and load balancer for the proxy service, respectively.
Since we will cover each of these in the `Load Balancer` section, for the sake of brevity, we will skip them here.
